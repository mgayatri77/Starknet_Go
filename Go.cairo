...
func get_liberties{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, 
    ...
    // Get board dimension and divide index by width
    let (dims) = board_size.read(); 
    let (_, local rem) = unsigned_div_rem(idx, dims[1]); 

    // Left edge of board, don't explore index-1 direction
    if (rem == 0) {
        ...
        let res = p1_val + p3_val + p4_val; 
        return (count=res);
    }

    // Right edge of board, don't explore index+1 direction
    if (rem == dims[1] - 1) {
        let (local p2_val) = get_lib_value(idx, idx-1, version, MAX_IDX); 
        let (local p3_val) = get_lib_value(idx, idx+dims[1], version, MAX_IDX); 
        let (local p4_val) = get_lib_value(idx, idx-dims[1], version, MAX_IDX); 
        let res = p2_val + p3_val + p4_val; 
        return (count=res);    
    }

    // Middle of board, explore all directions
    let (local p1_val) = get_lib_value(idx, idx+1, version, MAX_IDX); 
    let (local p2_val) = get_lib_value(idx, idx-1, version, MAX_IDX); 
    let (local p3_val) = get_lib_value(idx, idx+dims[1], version, MAX_IDX); 
    let (local p4_val) = get_lib_value(idx, idx-dims[1], version, MAX_IDX); 
...
// reset a chain of 0 liberty values of opposing player stones to empty
